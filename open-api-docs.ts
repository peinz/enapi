...
export const createOpenApiJsonDoc = (endpoints: Endpoints) => {
  ...
  const paths = Object.entries(endpoints).reduce((pathObj, [path, ep]) => {
    ...
    if (ep.definition.postBody) {
      pathCollectionActionObj.post = {
        tags: [path],
    ...
    if (ep.definition.patchBody) {
      pathEntityActionObj.patch = {
        tags: [path],
    ...
    if (ep.definition.remove) {
      pathEntityActionObj.delete = {
        tags: [path],
    ...
    if (ep.definition.collectionQueryParams) {
      pathCollectionActionObj.get = {
        tags: [path],
        summary: `get ${path} entities`,
        description: `get ${path} entities`,
        operationId: "getCollection" + path,
        parameters: Object.entries(ep.definition.collectionQueryParams).map((
          [name, def],
        ) => ({
          name,
          in: "query",
